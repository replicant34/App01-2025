...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        $stmt = $pdo->prepare("UPDATE Clients SET Company_type = ?, Full_Company_name = ?, Short_Company_name = ?, Contract_type = ?, Contract_number = ?, Contract_date = ?, INN = ?, KPP = ?, OGRN = ?, Physical_address = ?, Legal_address = ?, Bank_name = ?, BIK = ?, Settlement_account = ?, Correspondent_account = ?, Contact_person = ?, Contact_person_position = ?, Contact_person_phone = ?, Contact_person_email = ?, Head_position = ?, Head_name = ? WHERE Client_id = ?");
        $stmt->execute([$companyType, $fullName, $shortName, $contractType, $contractNumber, $contractDate, $inn, $kpp, $ogrn, $physicalAddress, $legalAddress, $bankName, $bik, $settlementAccount, $correspondentAccount, $contactPerson, $contactPersonPosition, $contactPersonPhone, $contactPersonEmail, $headPosition, $headName, $clientId]);

        // Log the edit action
        $logStmt = $pdo->prepare("INSERT INTO action_logs (user_id, action_type, table_name, description, ip_address) VALUES (?, 'EDIT', 'Clients', 'Editted client: $fullName', ?)");
