document.addEventListener('DOMContentLoaded', function() {
    ...
    editButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            fetch(`assets/get_courier.php?id=${courierId}`)
                .then(response => response.json())
                .then(data => {
                    document.getElementById('editCourierId').value = data.courier_id;
    ...
    deleteButtons.forEach(button => {
        button.addEventListener('click', function() {
            const courierId = this.getAttribute('data-id');
            document.getElementById('mc-confirmDelete').setAttribute('data-id', courierId);
    ...
    document.getElementById('mc-confirmDelete').addEventListener('click', function() {
        const courierId = this.getAttribute('data-id');
        // Send delete request
        fetch(`assets/delete_courier.php?id=${courierId}`, { method: 'DELETE' })
            .then(() => {
                location.reload();
            });
    });
    ...
    downloadCsvButton.addEventListener('click', function() {
        window.location.href = 'assets/download_couriers_csv.php';
    });
}); 
...
document.querySelectorAll('.delete').forEach(button => {
    button.addEventListener('click', function(event) {
        ...
        document.getElementById('mc-confirmDelete').onclick = function() {
            ...
            fetch('assets/delete_courier.php', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded',
                },
                body: new URLSearchParams({ courier_id: courierId })
            }).then(() => {
