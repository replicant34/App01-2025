document.addEventListener('DOMContentLoaded', function() {
    ...
    editButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            fetch(`assets/get_agent.php?id=${agentId}`)
                .then(response => response.json())
                .then(data => {
                    document.getElementById('editAgentId').value = data.Agent_id;
    ...
    deleteButtons.forEach(button => {
        button.addEventListener('click', function() {
            const agentId = this.getAttribute('data-id');
            document.getElementById('mc-confirmDelete').setAttribute('data-id', agentId);
    ...
    document.getElementById('mc-confirmDelete').addEventListener('click', function() {
        const agentId = this.getAttribute('data-id');
        // Send delete request
        fetch(`assets/delete_agent.php?id=${agentId}`, { method: 'DELETE' })
            .then(() => {
                location.reload();
            });
    });
    ...
    downloadCsvButton.addEventListener('click', function() {
        window.location.href = 'assets/download_agents_csv.php';
    });
}); 
