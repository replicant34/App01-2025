...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($fullCompanyName)) {
        ...
        try {
            $stmt = $pdo->prepare("INSERT INTO Agents (Company_type, Full_Company_name, Short_Company_name, INN, KPP, OGRN, Physical_address, Legal_address, Bank_name, BIK, Settlement_account, Correspondent_account, Contact_person, Contact_person_position, Contact_person_phone, Contact_person_email, Head_position, Head_name) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            ...
            // Log the action
            $stmt = $pdo->prepare("INSERT INTO action_logs (user_id, action_type, table_name, description, ip_address) VALUES (?, 'INSERT', 'Agents', 'Added new Agent: $fullCompanyName', ?)");
